...
try:
    secrets_manager = SecretsManager(
        ...
        Path(settings.words_list_path)
    )
except FileNotFoundError:
    print("Warning: Secrets configuration not found. Guess features will not be available.")
...
class GuessRequest(BaseModel):
    """정답 추측 요청"""
    word: str
    date: Optional[str] = None  # YYYY-MM-DD 형식, 없으면 오늘
...
async def guess_secret_word(request: GuessRequest):
    """정답 단어를 추측합니다."""
    if vector_db is None:
        raise HTTPException(
            status_code=503,
    ...
    if secrets_manager is None:
        raise HTTPException(
            status_code=503,
            detail="Secrets configuration not available. Please set up the data files first."
        )
    ...
    # 정답 단어 조회
    try:
        secret_word = secrets_manager.get_secret_word(target_date)
    except FileNotFoundError as e:
        raise HTTPException(status_code=500, detail=f"Secrets configuration error: {str(e)}")
